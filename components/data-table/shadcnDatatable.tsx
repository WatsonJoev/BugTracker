...
export const columns: ColumnDef<IssueTypes>[] = [
    ...
    // },
    {
        ...
        cell: ({ row }) => <div className="capitalize flex justify-end">
            <Link href={"/tracker/issues/update?id=" + row.getValue("id")}>
            ...
            {/* <Button variant="ghost" className="h-8 w-8 p-0"
            onClick={() => axios.put("/tracker/issues/board/" + row.getValue("id"), { status: "DELETE" })
                .then(response => {
            ...
            <Link href={"/tracker/issues/board/" + row.getValue("id")}>
                <Button variant="ghost" className="h-8 w-8 p-0">
